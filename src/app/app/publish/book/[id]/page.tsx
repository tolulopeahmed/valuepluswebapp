  ...
  return (
    <Modal open={open} onClose={() => !saving && onClose()}>
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        ...
        {program?.summary && <div className="mt-3 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-white/45">Total earnings from distributors<strong className="mt-1 block text-xl text-[rgb(var(--vp-accent-rgb))]">{naira(program.summary.distributor_author_earnings)}</strong><span className="mt-1 block text-[0.68rem]">Your net earnings from sales generated by your distributors.</span></div>}
        {!program ? <p className="mt-3 text-xs text-white/35">Loading…</p> : program.affiliates.length === 0 ? (
          ...
          <div className="mt-3 max-h-64 space-y-2 overflow-y-auto">
            {program.affiliates.map((affiliate) => (
              <div key={affiliate.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
                {affiliate.avatar ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={affiliate.avatar} alt="" className="h-14 w-14 shrink-0 rounded-2xl border border-[rgb(var(--vp-accent-rgb))]/50 object-cover" />
                ) : (
                  <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl border border-[rgb(var(--vp-accent-rgb))]/50 bg-white/5 text-xl font-black text-[rgb(var(--vp-accent-rgb))]">
                    {(affiliate.name.trim().charAt(0) || "D").toUpperCase()}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs text-white/45">
                    <span className="text-sm font-black text-white">{affiliate.name.trim().split(/\s+/)[0] || "Distributor"}</span> · {affiliate.email}
                  </p>
                  <p className="mt-1.5 text-[0.68rem] text-white/55">{affiliate.orders_referred} orders · {affiliate.units_sold} units · {naira(affiliate.revenue_generated)} sales</p>
                </div>
                <span className="shrink-0 text-sm font-black text-green-300">{naira(affiliate.earnings)}</span>
              </div>
            ))}
          </div>
        )}
